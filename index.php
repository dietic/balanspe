<?php include 'includes/header.php'; ?>
<div id="home" class="home-section valign-wrapper">
	<h2 class="">Profesionales en servicios<br> administrativos y contables</h2>
	<div class="slide-down-container">
		<h6 class="down-cat">Nosotros</h6>
		<a href="#about"><i class="slide-down-arrow center material-icons">keyboard_arrow_down</i></a>
	</div>	
</div>
<div id="about" class="row about-section">
	<!-- Columna izquierda -->
	<div class="about-left col s3 valign-wrapper">
		<h3>NOSOTROS</h3>
	</div>

	<!-- Columna derecha -->
	<div class="about-right col s9 valign-wrapper ">
		<div class="col s6 about-right-left">
			<p >Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas in maximus mi, venenatis fringilla ex. Nullam eu turpis blandit leo mattis imperdiet. Quisque nulla mauris, commodo sit amet nibh non, fermentum mollis nulla. Nulla facilisi. In pretium dictum porta. Fusce cursus ultrices metus porta tincidunt. Phasellus sodales rhoncus felis, eu laoreet justo vulputate eu. Vivamus sit amet turpis sed felis fringilla blandit vel ultrices lectus. </p>
		</div>
		<div class="col s6 about-right-right">
			<p >Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas in maximus mi, venenatis fringilla ex. Nullam eu turpis blandit leo mattis imperdiet. Quisque nulla mauris, commodo sit amet nibh non, fermentum mollis nulla. Nulla facilisi. In pretium dictum porta. Fusce cursus ultrices metus porta tincidunt. Phasellus sodales rhoncus felis, eu laoreet justo vulputate eu. Vivamus sit amet turpis sed felis fringilla blandit vel ultrices lectus. </p>
		</div>
	</div>

</div>







<div id="features" class="row features-section">
	<div class="row">
		<!-- Columna izquierda -->
		<div class="col s3 features-left valign-wrapper">
			<h3>VALORES</h3>
		</div>

		<!-- Columna derecha -->
		<div class="col s9 features-right">
			<div class="row features-right-top">
				<div class="col s4 features-swap-icon features-right-top-one">
					<img src="img/hand-shake-thin-white.png" alt="">
					<p>INTEGRIDAD</p>
				</div>
				<div class="col s4 features-swap-icon features-right-top-two" >
					<img src="img/circular-target-black.png" alt="">
					<p>OBJETIVIDAD</p>
				</div>
				<div class="col s4 features-swap-icon features-right-top-three">
					<img src="img/hand-shake-thin-white.png" alt="">
					<p>COMPROMISO</p>
				</div>

			</div>
			<div class="row features-right-bottom">
				<div class="col s4 features-swap-icon features-right-bottom-one ">
					<img src="img/hand-shake-thin-black.png" alt="">
					<p>RESPONSABILIDAD</p>
				</div>
				<div class="col s4 features-swap-icon features-right-bottom-two ">
					<img src="img/hand-shake-thin-white.png" alt="">
					<p>CONFIDENCIALIDAD</p>
				</div>
				<div class="col s4 features-swap-icon features-right-bottom-three">
					<img src="img/hand-shake-thin-black.png" alt="">
					<p>COMPETENCIA</p>
				</div>
			</div>
		</div>
	</div>
	<div class="slide-down-container">
		<h6 class="down-cat">Servicios</h6>
		<a href="#about"><i class="slide-down-arrow center material-icons">keyboard_arrow_down</i></a>
	</div>
	
</div>

<?php include 'includes/footer.php'; ?>